...
async function updateJson() {
    try {
        const filePath = path.resolve(process.cwd(), 'appoints-afterbreak.json');
        if (!fs.existsSync(filePath)) {
            console.error('appoints-afterbreak.json not found');
            return;
        }
        ...
        const updatedAppointments = appointments.map((appt: any) => {
            ...
            try {
                ...
                console.log(`Updated ${appt.patientName}: ${appt.date} ${appt.time} (Shifted +1.5h)`);
            } catch (e) {
        ...
        console.log('Successfully updated appoints-afterbreak.json: Added 1.5 hours to all fields');
    } catch (error) {
